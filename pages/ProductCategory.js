// const { clearChildrenWithQuerySelector, cloneTemplate } = require("./../lib/Helper")
                    ...
                    <div class="container-fluid py-1 px-3">
                        ...
                        <div class="collapse navbar-collapse mt-sm-0 mt-2 me-md-0 me-sm-4" id="navbar">
                            <div class="ms-md-auto pe-md-3 d-flex align-items-center">
                                ...
                                <button class="btn btn-success" style="margin-top: 10px; margin-left: 10px" id="addProductCategoryButton">Add</button>
                            </div>
                        </div>
                    </div>
                </nav>
                            ...
                            <form class="modal-body" id="modalproductCategoryForm">
								<div class="row">
									<div class="col-12 mb-3">
										<label for="nameBasic" class="form-label" style="font-size: 10pt">Category Name</label>
										<input type="text" id="categoryName" class="form-control p-3" placeholder="Enter category name">
									</div>
                                    <div class="col-12 mb-3">
										<label for="nameBasic" class="form-label" style="font-size: 10pt">Description</label>
										<input type="text" id="categoryDescription" class="form-control p-3" placeholder="Description">
									</div>
                                    <div class="col-12 mb-3">
										<label for="nameBasic" class="form-label" style="font-size: 10pt">Manufacturer</label>
    ...
    #_addEvents(){
        ...
        if (saveUpdateButton) {
            saveUpdateButton.addEventListener('click', (e) => {
                ...
                if (categoryName) {
                    ipcRenderer.send('saveProductCategory', {categoryName, categoryDescription, manufacturerName})
                }
            })
        }

        ipcRenderer.on('saveProductCategoryResponse', (event, response) => {
            if (response.success) {
                Swal.fire({
        ...
        ipcRenderer.on('fetchProductCategoriesResponse', (event, response) => {
            if (response.success) {
                ...
                for (let i = 0; i < response.data.length; i++) {
                    ...
                    html += String.raw`
                        <tr>
                            <td style="font-family: monRegular !important; font-size: 8pt">${category.name}</td>
                            <td style="font-family: monRegular !important; font-size: 8pt">${category.description}</td>
                            <td style="font-family: monRegular !important; font-size: 8pt">${category.manufacturer}</td>
                            <td style="font-family: monRegular !important; font-size: 8pt">${category.status == "active" ? 'Active' : 'Inactive'}</td>
                            <td class="text-end">
                                <button class="btn btn-secondary btn-sm" style="font-family: monRegular !important; font-size: 8pt" data-id="${category.id}" onclick="">Edit</button>
                                <button class="btn btn-danger btn-sm" style="font-family: monRegular !important; font-size: 8pt" data-id="${category.id}" onclick="">Delete</button>
                            </td>
                        </tr>
                    `
                ...
                console.log('Error fetching product categories: ', response.error);
            }
        })
    }
